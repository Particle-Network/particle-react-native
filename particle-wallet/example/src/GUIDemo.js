...
import { OpenBuyNetwork } from '@particle-network/rn-wallet';
...
export default class GUIDemo extends PureComponent {
    ...
    connectWithParticleAuthCore = async () => {
        try {
            ...
            particleWallet.createSelectedWallet(account.publicAddress, WalletType.AuthCore, "Custom Wallet");
            Toast.show({
                type: 'success',
    ...
    connectMetamask = async () => {
        try {
            ...
            particleWallet.createSelectedWallet(account.publicAddress, WalletType.MetaMask);
        }
        catch (e) {
            const error = e;
            console.log(error);
    ...
    render = () => {
        return (React.createElement(SafeAreaView, null,
            React.createElement(FlatList, { data: this.data, renderItem: ({ item }) => (React.createElement(TouchableOpacity, { style: styles.buttonStyle, onPress: () => {
                        this.setState({
                            currentKey: item.key,
                        });
                        item.function();
                    } },
                    React.createElement(Text, { style: styles.textStyle }, item.key))) }),
            React.createElement(ModalSelector, { selectStyle: { display: 'none' }, onChange: this.handleModelSelect, data: this.state.currentOptions, ref: (el) => {
                    this.modalSelect = el;
                } })));
    };
}
